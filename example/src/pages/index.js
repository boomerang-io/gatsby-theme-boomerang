    ...
    <main className={styles.container} id="content">
      <header className={styles.header}>
        <div className={styles.introContainer}>
          <h1 className={styles.introTitle}>{data.site.siteMetadata.title}</h1>
          <p className={styles.introSubtitle}>{data.site.siteMetadata.description}</p>
          <DocsSearch resultsAlignment="left" />
        </div>
        <nav className={styles.quickLinksNav}>
          {data.site.siteMetadata.homeNavigationLinks.map((navLink) => (
            <QuickLink key={navLink.text} title={navLink.text} href={navLink.path} />
          ))}
        </nav>
      </header>
      <article className={styles.article}>
        {data.site.siteMetadata.solutions.map((solution) => (
